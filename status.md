...
#item table :not(thead) > tr:nth-child(odd) { background: #1c1c1c } /* alternating rows, subtle (the original lightened the first row alone, which read as alternating with two instances) */
#item .instances table :not(thead) > tr:first-of-type td:first-child { border-left: 3px solid #4a7fc1 } /* the most recently focused instance per device (the app sorts by focus time): this tab on this device */
