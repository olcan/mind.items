#util/sample/examples/14 Confine [simulation](#util/sim) states to observed ([logged](#logger)) distributions. Weight accumulation or maximization can be used if maximum-weight (or maximum-likelihood) estimates are sufficient and over-concentrated posteriors and time-based stopping (based on `time` option, before `max_mks` is satisfied) are acceptable.
```js:js_input
