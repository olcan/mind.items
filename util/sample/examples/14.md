...
//   sleep_hour: { mean: sample(between(0,2)) , stdev: sample(between(0,2)) },
// })
const x = state(vars, params, { events:true })
const events = sleep_wake_events('olcan', x)
...
function sleep_wake_events(name, x, events=[]) {
  ...
  const around_wake_hour = _ => around(x.wake_hour.mean, x.wake_hour.stdev)
  const around_sleep_hour = _ => around(x.sleep_hour.mean, x.sleep_hour.stdev)
  events.push(set(wake(daily(around_wake_hour)), '_name', name + '.wake'))
  events.push(set(sleep(daily(around_sleep_hour)), '_name', name + '.sleep'))
  return events
}
// TODO: refactor this code to standardize event names and event/state associations
...
// TODO: also reconsider the output names, e.g. for mean_sleep_lengths
