...
simulate(x, now(), [
  wake(x.alice, daily(around(
    sample(between(6,10), 'wake hour mean'),
    sample(between(0,2), 'wake hour stdev')
  ))),
  sleep(x.alice, daily(around(
    sample(between(0,3), 'sleep hour mean'),
    sample(between(0,2), 'sleep hour stdev')
  )))
], { allow_next:true }) // simulate to next event >now
predict(last_event_name(x), 'next event name')
