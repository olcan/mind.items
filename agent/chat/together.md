#agent/chat/together responds using [Together Chat API](https://docs.together.ai/reference/chat-completions).
...
async function run_chat_agent(messages, config) {
  config.model ||= 'meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo' // https://docs.together.ai/docs/chat-models
  ...
  // convert messages to together (openai) format
...
const create_request = (messages, config) => ({
  ...
  body: JSON.stringify({
    ...
    // model: https://docs.together.ai/docs/chat-models
    messages,

    // https://docs.together.ai/docs/function-calling
    // example 'eval' tool, but only tool_choice is specified (can be 'auto')
