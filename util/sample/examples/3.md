#util/sample/examples/3 Sample `A∈[0,1]`, `B∈[a,1]` conditioned _implicitly_ on forked `B'>.9` using _likelihood weights_ `P(B'>.9|a)`. This can be viewed as averaging infinitely many forked condition indicators `(b'>.9 ? 1 : 0)` and is preferable when likelihood weights can be computed efficiently.
```js:js_input
sample(() => {
