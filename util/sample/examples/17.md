...
const events = {
  ...
  hunger: _at(randomly(1), inc('hunger', null)),
  eat:    _if('idle', randomly(1), x=>{
    if (x.hunger >= 5) { x.meals++; x.hunger = 0 }
    else if (x.hunger >= 3 && random_boolean(.5)) { x.snacks++; x.hunger-- }
    else return null // cancel eating
  }),
}
