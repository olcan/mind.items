...
const events = {
  wake:   _if('asleep', daily(within(9, 1)), 'idle'),
  sleep:  _if('idle',   daily(within(2, 1)), 'asleep'),
...
// TODO: what to do with these, and do you need a generic satisfies() thing similar to from() or can you just use from()? the challenge is you have a container state object x in this case
const and = (...fJ) => x=>fJ.every(f=>f(x))
const or = (...fJ) => x=>fJ.some(f=>f(x))
const not = f => x => !f(x)
