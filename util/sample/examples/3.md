#util/sample/examples/3 Sample `A∈(0,1)`, `B∈(a,1)`, `C∈(a,b)`. Confine `C∈(.4999,.5001)`. Confining to #///domains enables _distances_ to be used to guide samples _outside_ the domain. Conditioning allows more general boolean expressions but depends on finding at least one sample that satisfies the condition (which can take too long) unless a custom _relaxation function_ is defined (see example TODO) to guide samples that fail the condition.
```js:js_input
