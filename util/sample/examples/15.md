...
// TODO: is accumulation stable for inference? it seems like it could NOT be distributionally stable because it must converge around the maximum-likelihood point, and could well be subject to a slow drift towards it; but then there is a question of why it IS stable for the non-mixture model, perhaps because it is unimodal? and if unimodality is an issue, perhaps eliminating prior jumps could make sense for the mixture case, to force concentration around a single mode?
...
condition(x.early_wake_hour_mean < x.wake_hour_mean - .5)
...
accumulate( // accumulate weights across samples, may require 'time' option
...
const _sample_options = {
  ...
  // max_time:10000,
  time:10000, // necessary due to slow drift w/ accumulation
  table:true,
  plot:true,
  // size:1, // debug
  // sort:true,
