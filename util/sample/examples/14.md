...
// TODO: the simulation period (e.g. 10 days) affects results significantly and is actually arbitrary -- we are most interested in what happens _asymptotically_, NOT restricted by simulation length but rather time or data availability. Tying it to r seems to make sense on surface but in practice seems to create problems e.g. w/ moves being inhibited, so we need to be more creative.
let x = { 
  t: _6am - 10,
  awake: false,
  wake_hours: [],
