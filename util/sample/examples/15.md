...
let x = state({
  ...
  p_early: sample(between(.3,.7)),
  early_wake_hour_mean: sample(between(6,8)),
  early_wake_hour_stdev: sample(between(.5,1.5)),
  early_sleep_hour_mean: sample(between(-2,3)),
  early_sleep_hour_stdev: sample(between(.5,1.5)),
  wake_hour_mean: sample(between(7,10)),
  wake_hour_stdev: sample(between(.5,1.5)),
  sleep_hour_mean: sample(between(-2,3)),
  sleep_hour_stdev: sample(between(.5,1.5)),
})
...
// NOTE: no sign of mks convergence after 3 minutes! is it possible that this is about the inference being unable to meaningfully separate the two components and slowly drifting toward a single-component degenerate solution? what if we restrict the sizes to not be too uneven and the stdev not being too small? does not seem like this is sufficient! restricting movement also did not do much for convergence  -- next thing to try is to modify the priors to distinguish the two components more, and that did not help either, so it really feels like there is a fundamental issue w/ accumulated convergence in a mixture, no matter how constrained! one (perhaps only) possible explanation at this point is oscillations between modes caused by pure accumulation -- we would like to try a softer kind of accumulation, e.g. an exponential decay like (a*.9 + b - a), but before we can do that we need to get out of the way the problem of log_wr being replaced by _fork(), and thus the "log_rwJ_base" being lost
...
const _sample_options = {
  ...
  time:60000, // we do not expect to satisfy max_mks (default <=5)
  table:true,
  plot:true,
  // size:1, // debug
  async:true
