#util/sample/examples/4 Sample `A∈(0,1)`, `B∈(a,1)`, `C∈(a,b)`. Confine `C≈.5±.0001` using [domain](#///domains) `around(.5, .0001)`, enabling use of sampling densities as weights, effectively _conditioning on sampling distribution_ of `sample(around(.5, .0001))` over an unbounded domain `(-∞,∞) ≡ {is:'finite'}`. Sampling densities are defined for all domains that can be sampled as `sample(domain)`.
```js:js_input
