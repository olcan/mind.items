...
let x = state({
  t: _6am - 30, // start at 6am, X days ago (longer is more informative)
  awake: false, // assume asleep at 6am
  early: false, // early (wake) day?
}, {
  p_early: sample(between(0,1)),
  // parameters for early day
  early_wake_hour_mean: sample(between(6,11)),
  ...
  // parameters for non-early day
  wake_hour_mean: sample(between(6,11)),
  wake_hour_stdev: sample(between(0,2)),
  sleep_hour_mean: sample(between(-2,3)),
  sleep_hour_stdev: sample(between(0,2)),
}, {
  events:true // for wake/sleep hours, sleep length, and next_event below
})
// confine early wake times (mean±stdev) to be before non-early times
confine(x.early_wake_hour_mean + x.early_wake_hour_stdev,
  below(x.wake_hour_mean - x.wake_hour_stdev))
...
// notes on mks convergence
// strong (relative-tail) convergence does not happen even after 3 minutes
...
const sleep = ft => _if(awake, ft, x=>{
  x.awake = false
  x.early = random_boolean(x.p_early) // early day tomorrow?
})
