...
const wake_hour =  { mean: sample(between(6,10)), stdev: sample(between(0,2)) }
const sleep_hour = { mean: sample(between(0,3)),  stdev: sample(between(0,2)) }
simulate(x, now(), [
  wake(x.alice, daily(around(...args(wake_hour)))),
  sleep(x.alice, daily(around(...args(sleep_hour))))
