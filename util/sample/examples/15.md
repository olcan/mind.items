...
//
// EFFECTIVE IDENTIFIABILITY (w.r.t. posterior statistics)
// alt & non-alt parameters are swappable w/o affecting observed weights
...
// core issue: observations are invariant, but inferences/predictions are not
// component model asymmetry (e.g. normal vs triangular) can resolve issue
...
// should not be conflated with component distance or separation, see below
//
// DETAILED IDENTIFIABILITY (w.r.t. uniquely "true" parameters)
// certain subsets of the parameter space remain non-identifiable
...
// either can be avoided under conditions introduced as modeling assumptions
// considered harmless since non-identifiable region is known, can be ignored
// relevant posterior stats, inferences, predictions, etc should also be ignored
//
