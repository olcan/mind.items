...
const x = state({
  t: _6am - round(30 * max(.25, context.r)),
  alice: sleep_wake_state({
...
}).configure({ events:true }) // enable x._events used below
simulate(x, now(), sleep_wake_events(x.alice), { allow_next:true })
...
// parameter uniqueness (identifiability) condition is important
// note "shortcutting" for condition has minimal benefit
// prior asymmetry is not important (empirically)

const sleep_wake_state = ({alt, ...mods}) => state({
  _params: {
  ...
  awake: false, // assume asleep initially
  ...mods, // merge mods before alt in case p_alt is modified
  alt: alt ?? (({ p_alt }) => random_boolean(p_alt)), // can also be modified
  // apply weight (_log_w) to ensure uniqueness s.t. alt_wake < wake
  // otherwise weights are invariant to swapping of alt and non-alt params
  // posterior sample necessarily becomes arbitrary mix of the two variants
  // hard to work with, e.g. param statistics (e.g. mean) become meaningless
  _log_w: ({ alt_wake_hour, wake_hour })=>
    log(alt_wake_hour.mean + alt_wake_hour.stdev*.5 <
     wake_hour.mean - wake_hour.stdev*.5)
})
function sleep_wake_events(x, mods={}) {
  ...
  return name_events({
    ...
    [path(x,'sleep')]: _if(_=>x.awake, sleep_sched, _=>{
