#util/sample/examples/13
```js:js_input

sample_array(A, alloc, summand(1), 'crypto stocks bonds reits')
let x = simulate({ alloc, prices:ones(A), value:1 }, 10, events)
maximize(x.value)

```
```js:js_removed

const A = 4 // assets
const alloc = [1,0,0,0] // allocation (optimal)
const μA = [.05, .025, .005, .0125] // mean change (relative) per period
// TODO: think about the fact that both maximize & minimize prefer higher variance because it assigns higher probability to extreme outcomes ... this feels like it is lacking some kind of balance; do we need a utility function of the outcome that can assign more weight to smaller wins and less weight to larger losses, thus rewarding consistent small wins instead of rare big wins? how else do we control outcomes?
// const μA = [.05, .05, .05, .05]
const σA = [.1, .05, .01, .025] // standard deviation in change per period
const dA = array(A, a=> μA[a]-.5*σA[a]**2)
const reprice = _do(x=>{
  apply(x.prices, (x,a)=> x * exp(dA[a] + σA[a]*random_normal()))
  x.value = sum(A, a=> x.alloc[a] * x.prices[a])
})
const events = name_events(()=>reprice)

const _sample_options = { 
  params:{A},
  stats:'ess wsum r t ua.crypto median.x.value',
  max_time: 3000,
  table:true,
  plot:true,
  // log:true,
  // size:1, // debug
}

```
#_util
