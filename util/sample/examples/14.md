...
const h_wake =  { μ: sample(between(6,10)), σ: sample(between(0,2)) }
const h_sleep = { μ: sample(between(0,3)),  σ: sample(between(0,2)) }
simulate(x, now(), [
  wake(x.alice,  daily(around(...args(h_wake )))), // alice wakes daily
  sleep(x.alice, daily(around(...args(h_sleep))))  // alice sleeps daily
